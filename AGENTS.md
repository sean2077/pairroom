...
- Read Store schema 10/provisioning 3 as embedded and schema 11/provisioning 4 with explicit immutable `host_mode`. New Rooms of either host mode write 11/4; existing 10/3 Rooms keep their original bytes and append schema. Registry checkpoint schema 2 and its strict shape stay unchanged. Reject retired Rooms before replay or repair; never infer missing collaboration, selections, or bindings from current defaults. No Legacy Room import, binding completion, role switching, or reviewer snapshot workspace remains. Preserve current-schema collaboration instruction versions verbatim.
