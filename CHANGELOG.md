...
- Desktop packages now ship the PairRoom CLI next to the host (`pairroom.exe` in the Windows NSIS installer, `/usr/local/bin/pairroom` in the Debian package, and `Contents/MacOS/pairroom` in the macOS app). CI no longer treats a lone `PairRoom.exe` as a complete Windows artifact. If no daemon is installed, the desktop host installs one from the bundled CLI instead of embedding a competing Service.
