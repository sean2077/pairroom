...
- Stop Embedded Event Logs from growing quadratically with tool calls. Each `turn.summary.updated` record is a complete Turn summary, and it used to be written and fsynced on every tool, plan, diff and usage event; one measured Room had 92% of a 14.7 MB log in these records. Summaries now persist immediately only at creation, turn start, final text, errors and terminal states, and otherwise at most every 30 seconds per unfinished Turn, when its adapter stops, or when the Room closes, while the browser still receives every update as a live sequence-zero event (diff/usage throttled to one per second). Item payload copies are limited to 4 KiB and a summary to 256 KiB; complete tool payloads remain in their own durable runtime events. Windowed snapshots now carry the 40 most recent, all unfinished and all message-correlated Turns (older ones arrive with message history pages), omit summary payloads from the event tail, and bound that tail to 4 MiB. A crash may lose up to the last 30 seconds of an unfinished Turn's summary items, never its raw events. No schema change: existing logs replay unchanged and are not rewritten, and older binaries read new logs.
