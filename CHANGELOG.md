...
- Make `pairroom relay install` runnable from any terminal (no native session required) and select multiple harnesses: `--runtime` accepts a comma-separated `cc|claude,codex,grok`, inside a recognized session it infers the harness, an interactive terminal without `--runtime` prompts a multi-select, and non-interactive use without `--runtime` fails listing the options instead of hanging. Grok Build reuses Claude Code's `.claude` hooks, so selecting Claude Code covers Grok; install prints that hint and writes no separate Grok hook, and native Grok relay remains unsupported. `pairroom relay bind` run outside a native session now fails with one clear in-session hint before any endpoint or workspace I/O.
