...
- Ship the `pairroom-relay` skill as a distributable asset (`assets/skills/pairroom-relay/`, `npx skills add sean2077/pairroom`); `relay install` embeds the same canonical file, kept byte-identical by a freshness test, and the slimmed skill defers protocol depth to the bind bootstrap. Add zero-flag onboarding: `bind --create` infers the creator slot from the recognized harness, zero-flag `bind` resolves the workspace's sole native Room and the caller's slot, `--slot 1|2` becomes the primary Agent-number form (durable `claude`/`codex` IDs remain accepted), and every ambiguity fails with candidates instead of guessing.
