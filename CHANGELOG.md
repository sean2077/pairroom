...
- Keep every Native Stop reply while the Service is stopped, not only the first. The local relay state held one pending reply, so a later Stop before the Service returned was lost. The slot now saves up to eight unpublished replies in order, each under its own sequence before any Service call, and the next hook or `relay reconcile` publishes them in that order under their original sequences once the Service is back. Only the oldest reply can have been sent: if its outcome is uncertain it is still reconciled by its original key and the replies behind it wait, so nothing is replayed under a new identity. `reconcile --resend` and `--discard` still decide only that oldest reply. A Stop beyond eight is not retained, consumes no sequence, and the hook says so on stderr with the `relay send` recovery; `relay status` reports `held_publications`. Relay state stays schema 2 and an older single-pending state loads unchanged; do not run an earlier CLI on a slot that still has held replies, because it does not know them. No Store, protocol or Service change.
