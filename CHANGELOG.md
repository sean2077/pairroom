...
- Serialize Native browser recovery across windows. Each save, clear or Forget of the local outbox record now holds a short per-Room Web Lock, never across a network request, and fails before publication if the lock is busy or Web Locks or writable localStorage are unavailable. Forget removes only the exact record present when its confirmation opened, so a newer draft from another window survives. API requests time out after 30 seconds, including the response body; a timed-out send keeps its original ID for an explicit check or retry, and receipt checking and Retry no longer run concurrently in one page. Publishing now requires a browser with Web Locks. Reload all open Native pages after upgrading so every window uses the same locking; existing outbox records keep their format.
