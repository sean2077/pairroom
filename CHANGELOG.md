...
- Automate WinGet publication of the Windows desktop installer: on `v*` tags the desktop workflow renders the `inno` manifest (schema 1.12.0, machine scope, SHA256 computed from the built artifact and cross-checked against the collected `SHA256SUMS`) and submits it to `microsoft/winget-pkgs` through a fork pull request using the new `WINGET_TOKEN` repository secret (classic PAT, `public_repo`). Rendering is deterministic and test-guarded (`desktop/scripts/test_winget_manifest.py`), reruns are idempotent per version, and submission failure never rewrites the Release.
