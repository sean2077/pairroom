...
- `pairroom-protocol/v5` makes the other participant's exact runtime-derived `mention_handle` the sole Agent-relay signal. Unique runtimes use `@claude`, `@codex`, or `@grok`; duplicate runtimes use stable slot-order `0/1` suffixes in handles and display names. Matching is case-insensitive and ignores code, URLs, and email; `@user` wins and self-mentions do not route. Greeting or introducing participants is not a human decision, so bootstrap tells Agents to omit `@user` and mention the peer. A reply that still includes `@user` alongside a peer handle stays with the human and records a visible notice. No peer mention ends relay, and either Agent may deliver the final result.
