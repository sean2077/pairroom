...
.PHONY: build install test race vet fmt check agent-contract release-contract cover stop dev run demo smoke release package desktop-build desktop-package desktop-update desktop-check clean docs-check browser-check js-check vuln vuln-binary lint lint-install
...
check: lint test race vet agent-contract release-contract docs-check desktop-check js-check
