...
- Reachability across turns: a 30-second park window after each turn collects fast answers; a Claude Code session can leave a background `relay wait` pending and its own harness wakes it when a message lands; in a wake-enabled Room the Service itself wakes a deep-idle Codex peer through the vendor-sanctioned `codex queue` surface (fixed body-free nudge, rate-limited, audited, fail-closed), and the CLI prints a human-executed template as the fallback for disabled Rooms or failed wakes. PairRoom never starts or interrupts agent sessions.
