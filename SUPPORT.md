...
Native host mode is experimental. The implementation supports Claude Code, Codex and Grok Build Native sessions with approved project hooks, nonce association, durable relay, explicit send/wait/exchange, stdout acknowledgement and bounded park. Grok's clipped hook channel uses a readiness-only continuation followed by foreground collection; clipped outgoing replies require explicit publication. See [Grok Native](docs/CLI_REFERENCE.md#grok-build-native). It does not yet have authenticated Codex Desktop ↔ Claude Code multi-round acceptance evidence from this development environment. Zero-hook binding, live attach, concurrent resume injection and scheduled idle self-wake are not supported. Grok vendor/tool E2E is likewise unverified here.
