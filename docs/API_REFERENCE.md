...
`POST /api/v1/relay/{room}/{slot}/{action}` has separate relay authentication, not browser-cookie or management-token authority. It requires `Authorization: Relay <secret>` and the CLI's binding/generation/official-session headers; credentials are loaded from an owner-only file, not command arguments. Actions are `inspect`, `associate`, `report`, `publication`, `send`, `wait`, `ack`, `status`, `summary`, `peer`, `failure`, `park`, `unbind`, and `upload`. `summary` is the body-free transport inspection used by `relay status --brief` / `relay reconcile --brief`: inbox counts and at most eight unknown-delivery recovery IDs, without message bodies, native session/transcript references, or the audit log. Before association only the narrow inspection/nonce/revocation paths and this pending-binding summary are usable. Every operation rechecks the live generation. Body limit is 256 KiB UTF-8 before JSON escaping; HTTP JSON is bounded to 2 MiB. Wait is bounded to 30 seconds and performs no claim while idle. For a Grok receiver with `park:true`, it only tests inbox readiness: `{"claim":null,"foreground_required":true}` requests a short hook continuation, without releasing a body or changing a message to delivering. The existing `park:false` foreground wait performs the actual FIFO claim. Only the CLI acknowledges after writing the full envelope to stdout.
