...
`POST /api/v1/diagnostics` is protected by the existing Management authentication, same-origin, and browser CSRF checks. There is no GET-triggered probe. An environment request is `{"mode":"environment"}`; an actual model check requires `{"mode":"runtime","actor":"slot2","confirm":true}`. `actor` is the stable slot (`slot1` or `slot2`), not a Runtime kind; numeric `1`/`2` are parse-time aliases. Both modes optionally accept `room_id`; otherwise the saved default Agent pair profile or Service defaults are used. A Room's stored selections never follow later default changes. Invalid Room selections are rejected rather than replaced with Service defaults.
