...
Desktop startup never installs a daemon. Desktop launch-at-login registration is changed only by the explicit native Settings control; the OS is its persistent authority, independent of daemon installation. The native bridge accepts only bounded startup get/set messages from the connected Management origin.
...
The Room renderer owns Activity disclosure/focus/scroll state, keyed by complete Turn/event identity rather than translated or truncated labels. The EventSource wrapper only batches transient delivery and preserves ordering; it must not inspect or repair Activity DOM. Unchanged cards stay attached, hidden Inspector tabs defer reconciliation, and text deltas accumulate in drafts rather than evicting the diagnostic tail. In-flight participant/message actions live outside their replaceable buttons. Settings drafts are distinct from shared snapshot state; a successful Save is confirmed by a post-write read, with newer edits retained.

Read-side cloning belongs in `internal/room/projection.go`; native Turn summary projection belongs in `turn_projection.go`. Non-projecting telemetry exits before scanning/cloning Turn history. The recent durable event tail retains the same bounded, ordered contents through amortized slice growth, clearing evicted payload references. Public snapshots detach optional timestamps as well as maps, slices, and raw JSON. These are in-memory projection changes, not truncation of the Event Log or model-facing context.

Retry admission uses the existing Room routing lock and durable Message projection: one source/participant can have at most one direct waiting/working retry child. Completed/failed/cancelled children do not permanently lock the source. This prevents concurrent duplicate retries without treating equal text as the same request or promising exactly-once semantics across crashes.
...
The Management Shell is Room-centric: the sidebar groups by Project, and in-app tabs embed an active Room View through the Management same-origin surface gateway (`/api/v1/rooms/{room}/surface/…`). The iframe uses the Management Session Cookie. The gateway injects the Runtime bearer on the server; the Runtime token never enters the DOM. Its explicit asset and API allowlist must cover the Room entry point and current native permission/lifecycle controls; contract tests reject missing entry-point assets and retired role-change paths. An in-app tab is not a Runtime lease. A live Room HTTP/SSE connection delays idle suspend; background tabs without a live request still obey idle / capacity / LRU / explicit-suspend constraints; switching back to a suspended tab requests activation again. An archived Room cannot open as a tab; restore it first.
