...
Readers accept only Store schema 12/provisioning 5 with explicit immutable `host_mode`. Earlier Room formats and Service roots are retired before replay, repair, recovery, or checkpoint rewrite; users create a new data root and recreate Rooms and profiles. Registry checkpoint schema 3 has strict canonical slot keys and retains host mode. Missing collaboration records and inferred Agent selections are rejected without migration or metadata rewriting. [Storage](STORAGE.md) owns replay details and [Upgrading](UPGRADING.md) owns retirement actions.
...
Durable slot IDs are `slot1` and `slot2`; RuntimeKind is independent. Configuration, routing, permission translation, and resume behavior must use the selected Runtime rather than infer a vendor from a slot. `claude` and `codex` remain relay CLI input aliases only.
...
A CLI-generated secret stays in the slot's owner-only credentials file; the Event Log contains only its hash. Binding stdout carries only a public binding reference. A private unconfirmed bind attempt stays outside active discovery and cannot overwrite committed local credentials before Service confirmation; retry preserves its original identity. Bind runs as a tool call inside the native session, captures the official session ID from the harness environment (`CLAUDE_CODE_SESSION_ID` / `CODEX_SESSION_ID`), and associates immediately; the approved Stop hook re-confirms that same official session identity before publication or collection, failing closed on mismatch, and records the transcript path the environment does not carry. Missing hooks, competing sessions with a different identity, cross-slot credentials and revoked generations are rejected. File access by another same-user process is outside the isolation claim. Project trust and native hook approval are not granted by PairRoom.

Stop publication and receive-side park are independent: persisting a reply does not wait for peer output. The CLI records one bounded pending publication before submission and reconciles ambiguous results with the original key. Collection records `delivering` before returning an envelope, then requires an explicit stdout-after-write acknowledgement. Recovery never automatically replays an uncertain delivery. [Protocol](PROTOCOL.md#native-host-protocol-v8) and [Storage](STORAGE.md#native-relay-state) specify the boundaries.
