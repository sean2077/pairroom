...
| `status` / `peer` | Bounded body-free delivery summary by default (`--brief=false` for history) / optional peer session and Runtime metadata. `status` first reconciles like `reconcile`, so it publishes a pending Stop reply and any held behind it; use `doctor` or `history` for read-only inspection |
