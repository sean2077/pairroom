...
| `doctor` | Read-only current Native Room diagnostics plus local hook installation, CLI/protocol version match and last hook observation. Reports an already active Native Room; a Room suspended by the Service idle timeout or a Service restart fails instead of being activated. Contacts no model; approval and model acceptance remain unknown. |
