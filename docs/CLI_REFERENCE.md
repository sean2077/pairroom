...
| `send --id <client-id> --text <body>` | Explicit message to peer; requires the bind-time association like every collection call; repeat the same ID after an uncertain response, never deduplicate by body. A queued receipt prints a body-free diagnostic and peer `wait` command; for an authenticated Codex target with a known vendor session it also prints a human-executed `wake_command` and `wake_notice`, never an automatic injection |
...
`wake_command` and its adjacent `wake_notice` are only a copyable Codex CLI template and boundary reminder for a human to decide whether to run. PairRoom never executes it. The command contains the target's vendor session identity in the local process arguments and a fixed body-free nudge; do not place peer message content, secrets, or additional shell fragments into that command.
