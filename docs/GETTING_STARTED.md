...
Start a non-Mock Service or use the one Desktop already owns. In the intended repository, install the relay hooks for the runtimes you will use. `relay install` runs from any terminal in that worktree (it does not need a native session); pass a comma-separated `--runtime`, or run it at a terminal without `--runtime` to pick from a multi-select. Grok Build reuses Claude Code's hooks, so selecting Claude Code covers Grok (native Grok relay is not supported):
