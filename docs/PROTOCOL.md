...
The hook authenticates each relay call with the bound session, so it contacts the Service only to confirm a new transcript reference, publish and park. It saves the reply WAL before any Service call, so a reply given while the Service is stopped stays pending for the next hook, `relay status` or `relay reconcile`, each of which publishes the saved replies; `relay doctor` and `history` inspect without publishing. A later Stop while an earlier reply is still unresolved is saved behind it under the next sequence, up to eight unpublished replies per slot whose saved state fits under about 1.9 MiB (seven replies of the 256 KiB maximum always fit), and is reported only after every earlier one settles, so publication keeps sequence order. Only the oldest reply can have been attempted; if its outcome is uncertain it is reconciled by its original key and the replies behind it wait. A Stop beyond either bound is not retained, consumes no sequence and says so on stderr; publish it explicitly with `relay send` if it matters.
