...
Reachability across turn boundaries is layered, and each layer is honest about its limit: a 30-second park window after a turn that addressed its peer or `@user` collects immediate answers; a Claude Code session can leave a harness-tracked background `relay wait` pending, and its own harness wakes it when a message lands; a deep-idle Codex session is woken by the Service through the vendor-sanctioned `codex queue` surface in a wake-enabled Room (fixed body-free nudge, rate-limited, audited, never automatically retried), with the human-executed template the CLI prints as the fallback for disabled Rooms or failed wakes ([verified surfaces](NATIVE_RELAY.md#verified-vendor-wake-surfaces)). PairRoom never starts or interrupts agent sessions, and queued messages survive restarts instead of being lost.
