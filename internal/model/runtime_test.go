...
func TestParticipantIdentitiesUseRuntimeHandlesAndStableDuplicateSuffixes(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			runtimes:   map[ActorID]RuntimeKind{ActorSlot1: RuntimeClaude, ActorSlot2: RuntimeCodex},
		...
		},
		{
			...
			runtimes:   map[ActorID]RuntimeKind{ActorSlot1: RuntimeGrok, ActorSlot2: RuntimeClaude},
		...
		},
		{
			...
			runtimes:   map[ActorID]RuntimeKind{ActorSlot1: RuntimeClaude, ActorSlot2: RuntimeClaude},
		...
		},
		{
			...
			runtimes:   map[ActorID]RuntimeKind{ActorSlot1: RuntimeCodex, ActorSlot2: RuntimeCodex},
		...
		{
			name:       "duplicate Grok",
			runtimes:   map[ActorID]RuntimeKind{ActorSlot1: RuntimeGrok, ActorSlot2: RuntimeGrok},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if got[ActorSlot1] != test.wantFirst || got[ActorSlot2] != test.wantSecond {
