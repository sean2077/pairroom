...
func TestNativeAttachmentIdentityAndQuotedImages(t *testing.T) {
	...
	original, err := e.Send(auth[model.ActorSlot1], SendRequest{ID: "image-source", Text: "image source", AttachmentIDs: []string{saved.ID}})
	if err != nil {
		t.Fatal(err)
	}
	quoted, err := e.SendUser(SendRequest{ID: "quote-image", To: model.ActorSlot1, Text: "inspect it", QuoteID: original.ID, AttachmentIDs: []string{saved.ID}})
	if err != nil || len(quoted.Attachments) != 1 || quoted.Quote.Text != original.Text {
		t.Fatalf("quote: %+v %v", quoted, err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	claim, err := e.Claim(ctx, auth[model.ActorSlot1], false)
	...
	if err := e.Ack(auth[model.ActorSlot1], claim.ID, claim.Receipt); err != nil {
		t.Fatal(err)
	}
	...
	if claim, err := e.Claim(ctx, auth[model.ActorSlot2], false); err == nil || claim != nil {
	...
	if _, err := e.SendUser(SendRequest{ID: "bad-quote", To: model.ActorSlot1, Text: "changed", QuoteID: original.ID}); err == nil {
