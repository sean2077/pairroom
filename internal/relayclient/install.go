...
Run ` + "`pairroom relay bind --room <id> --slot <claude|codex>`" + ` in the Room's project. Slot IDs are not runtime names. If missing, install the reported runtime's project hooks and have the user approve them; never bypass native trust. Echo the returned one-time nonce in your visible reply once. Keep the returned bootstrap/collaboration instructions as this session's protocol. Credentials are read by the CLI; never read or print the credential file.
