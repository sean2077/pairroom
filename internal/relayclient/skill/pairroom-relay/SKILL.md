...
**Stay reachable** only while a joint task is active: when the harness wakes on tracked background completion, keep exactly one background `pairroom relay wait --timeout 300`. Resolve its result before another collector. Re-hang only after real input or explicit confirmation to continue, never after an empty timeout; cancel at task end. A poll-only harness needs explicit polling in the active turn. Stop parking is bounded, not deep-idle wake. An idle peer needs its next turn or a human nudge; agents never run a printed vendor wake command.
