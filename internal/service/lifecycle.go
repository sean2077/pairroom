...
func (r *Registry) RenameRoom(ctx context.Context, roomID, name string) (Room, error) {
	...
	return r.mutateRoom(ctx, roomID, EventRoomRenamed, roomRenamedPayload{Name: name, UpdatedAt: updatedAt}, func(room *Room) {
		room.Name = name
...
func (r *Registry) ArchiveRoom(ctx context.Context, roomID string) (Room, error) {
	updatedAt := r.now()
	return r.mutateRoom(ctx, roomID, EventRoomArchived, roomLifecyclePayload{Lifecycle: RoomArchived, UpdatedAt: updatedAt}, func(room *Room) {
...
func (r *Registry) RestoreRoom(ctx context.Context, roomID string) (Room, error) {
	updatedAt := r.now()
	return r.mutateRoom(ctx, roomID, EventRoomRestored, roomLifecyclePayload{Lifecycle: RoomActive, UpdatedAt: updatedAt}, func(room *Room) {
		room.Lifecycle = RoomActive
		room.UpdatedAt = updatedAt
	})
}

func (r *Registry) mutateRoom(ctx context.Context, roomID, eventKind string, payload any, apply func(*Room)) (Room, error) {
	r.provisionMu.Lock()
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	// The Room Event Log is already committed. From this point onward we retain
	// the new projection and fail closed if the disposable checkpoint cannot be
