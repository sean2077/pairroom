...
func TestManagementShellAuthenticationAssetsAndSecurityHeaders(t *testing.T) {
	...
	for _, forbidden := range []string{"sessionStorage", "window.prompt(", "window.confirm("} {
	...
	if shell.Code != http.StatusOK || !strings.Contains(shell.Body.String(), `/management-ux.css`) ||
		...
		!strings.Contains(shell.Body.String(), `class="sidebar-top"`) ||
		!strings.Contains(shell.Body.String(), `id="sidebar-collapse"`) {
